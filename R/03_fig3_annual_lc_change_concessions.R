...
render_and_save_all_concessions <- function(hti_gav_annual_lc_df, output_dir) {
  ...
  for (concession_ in concessions) {
    ...
    p <- ggplot(filtered_df, aes(year, area_ha)) +
      geom_area(aes(fill = as.factor(class_desc)), position = position_stack(reverse = FALSE)) +
      ...
      scale_fill_manual(
        values = c("Forest" = "#009E73", "Non-forest" = "#F0E442", "Cleared for pulp" = "#CC79A7"),
        breaks = non_zero
      ) +
      guides(
    ...
    file_path <- file.path(output_dir, paste0(gsub(" ", "_", concession_), "_TreeMap_AnnualChanges.png"))
