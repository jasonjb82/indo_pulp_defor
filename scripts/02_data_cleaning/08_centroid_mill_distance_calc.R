...
kali_mill_dist <- kali_cp_clean_df %>%
  ...
  mutate(dist_land_kalimantan_km = dist_km.x,
    dist_land_sumatera_km=dist_km,
    dist_sea_km=dist_km.y,
    dest.y = ifelse(is.na(dest.y),dest.x,dest.y)
  ) %>%
  select(id=orig.x,mill=dest.y,dist_land_kalimantan_km,dist_land_sumatera_km,dist_sea_km) %>%
  print()

kali_mill_dist_add <- kali_cm_clean_df %>%
  select(id=orig,mill=dest,dist_land_kalimantan_km=dist_km) %>%
  mutate(dist_land_sumatera_km=0,dist_sea_km=0) 
...
cm_dist_add_df <- kali_mill_dist %>%
  bind_rows(kali_mill_dist_add) %>%
  bind_rows(suma_mill_dist) %>%
  print()
...
cm_dist_add_final_df <- cm_dist_df %>%
  replace(is.na(.), 0) %>%
  mutate(total_dist = dist_land_kalimantan_km + dist_land_sumatera_km + dist_sea_km) %>%
  group_by(id) %>%
  slice_min(total_dist, n = 1, with_ties = FALSE) %>%
