...
| RPBBI wood supply 2015–2019 | `remote/01_data/01_in/wwi/RPBBI_2015_2019_compiled.xlsx` | Non-spatial | xlsx | WWI / RPBBI (MoEF reporting) | 08_calc_mai.R | Compiled wood-supply records per supplier-year |
| RPBBI wood supply 2020 | `remote/01_data/01_in/wwi/RPBBI_2020_compiled.xlsx` | Non-spatial | xlsx | WWI / RPBBI | 08_calc_mai.R | Single xlsx, same schema as 2015–2019 |
| RPBBI wood supply 2021 | `remote/01_data/01_in/wwi/RPBBI_2021_compiled.xlsx` | Non-spatial | xlsx | WWI / RPBBI | 08_calc_mai.R | Single xlsx, same schema |
...
| Merged wood supply 2015–2022 | `remote/01_data/02_out/tables/ws_merge_clean_2015_2022.csv` | Non-spatial | csv | Intermediate output (consolidated RPBBI) | 12_paper_stats.R | Single CSV replacing four annual RPBBI xlsx files |
| Indonesia LCC workbook (Gaveau, Kalimantan tab only) | `remote/01_data/01_in/gaveau/IDN_2001_2022 landcover change of Oil Palm and Pulpwood_05JUNE2023.xlsx` | Non-spatial | xlsx | Gaveau / TreeMap | 12_paper_stats.R | Multi-sheet xlsx; only `PULPWOOD EXPANSION` sheet (`skip=5, n_max=22`) is now read |
| Annual expansion stats (TreeMap) | `remote/01_data/02_out/tables/id_annual_expansion_stats_ttm.csv` | Non-spatial | csv | Intermediate output (replaces Gaveau xlsx) | 12_paper_stats.R | Annual national pulp + palm forest/non-forest expansion |
...
**Audit deltas vs. prior version (work has tightened up several inputs):**

- `samples_landuse_ttm.csv` and `samples_gfc_ttm.csv` are now single CSVs; the prior folder-of-CSVs reads (`gee/gaveau/*gaveau_classes.csv`, `gee/gfc_ttm/*.csv`) have been retired.
- `12_paper_stats.R` no longer reads the four annual `RPBBI_*.xlsx` files directly — it now reads the consolidated `ws_merge_clean_2015_2022.csv` (RPBBI xlsx files are still used by `08_calc_mai.R`).
- `12_paper_stats.R` no longer reads the Gaveau `IDN_2001_2022 ... xlsx` for the main pulp/palm trend stats — those are now in `id_annual_expansion_stats_ttm.csv`. The workbook is still read once for the Kalimantan-only tab.
- `RPBBI_2022_compiled.xlsx`, `pulp_exports_wits.csv`, and the `annual_pulp_shr_prod.xlsx` reference in `12_paper_stats.R` have been removed from the active scripts (the production xlsx is still used by `01_summary_figure.R`).
...
### 1. Consolidation
- **RPBBI wood supply (2015–2019, 2020, 2021)** — three near-identical xlsx files are read separately in `08_calc_mai.R` and `rbind`ed. The downstream `12_paper_stats.R` already consumes a consolidated `ws_merge_clean_2015_2022.csv` — point `08_calc_mai.R` at the same file and retire the three xlsx inputs.
- **GAEZ grid + GAEZ HTI** (`gaez_grid_share.csv`, `gaez_hti_areas.csv`) — both feed the same aggregated 4-class scheme in `17_defor_elasticity.R`; precompute the aggregated columns and ship a single slim file per scope (grid, HTI).
...
- **Indonesia LCC workbook (Kalimantan tab)** — only ~22 rows of one sheet are read with hard-coded `skip=5, n_max=22`. Replace with a tiny `kali_pulp_forest_loss.csv` instead of shipping the full xlsx.
...
- **RPBBI files (08_calc_mai.R)** — only `YEAR, SUPPLIER_ID, VOLUME_M3` are read; drop everything else (or just use the consolidated CSV — see §1).
- **Fastmarkets pulp prices** — only 3 series columns are kept (`fp_plp_0045 / 0053 / 0056`); drop other series and reformat to long.
- **`IUPHHK_HTI_TRASE_20230314_proj.shp`** — scripts only use `ID` and `namaobj` (plus geometry). Drop all other attributes; consider also simplifying geometries (e.g. `ms_simplify` to ~50–100 m tolerance) since outputs are used at 1 km / 10 km grid scale.
...
- **`pulp_exp_model_var_1km_*.csv`** — these are by far the heaviest tables (1 km × Sumatra+Kalimantan × ~25 covariates + 64 `ya_*` spectral anomaly indices). The `ya_*` indices dominate file size; check whether all 64 actually drive the RF post-tuning, and prune any that variable-importance shows as negligible.
...
- **HTI annual land-use change** (`hti_land_use_change_areas.csv`) drives a per-concession plot loop — already aggregated; fine.
- **Validation spreadsheet** (`Validation_11classes...xlsx`) — only the `Points` sheet and `Inputs!A6:C17` ranges are used. Ship those as two tiny CSVs (`validation_points.csv`, `mapped_areas.csv`).
- **Fastmarkets daily prices** — script collapses to annual means; ship the annual series rather than daily.

### 4. Reformatting
- **All `.xlsx` reads**: every Excel file in this audit is treated as a flat table. Convert them all to `.csv` (or `.csv.gz`). Excel parsing also forces `skip=` magic numbers (Fastmarkets `skip=4`, Gaveau Kalimantan tab `skip=5, n_max=22`, Validation `range="A6:C17"`) that are brittle. CSV inputs avoid this entirely.
- **Heavy intermediate panels** (`pulp_exp_model_var_1km_*.csv`, `tbl_long_pulp_clearing_gfc_forest.csv`, `samples_*_ttm.csv`): ship as **Parquet** (`arrow::write_parquet`) — typically 5–10× smaller than CSV with native typing, and `read_parquet` is drop-in.
...
### 5. General Feedback
- **Path standardization**: `wdir` is set inconsistently — some scripts use `"remote"`, some `"remote/"`. `02_deforestation_timing.R` uses backslash paths (`"\\01_data\\02_out\\..."`) that will break on Linux/macOS reviewer machines. Standardize on forward slashes (`file.path()` or `here::here()`).
- **`memory.limit(size=60000)`** has already been removed from `01_summary_figure.R` (was Windows-only and is gone in R ≥ 4.2) — good. Audit other scripts for similar Windows-only calls.
...
- **Intermediate-vs-input ambiguity**: several files in `01_data/02_out/...` are *intermediate outputs* produced by upstream scripts that are **not** in the replication folders (e.g. `hti_harvest_yr.csv`, `tbl_long_pulp_clearing_gfc_forest.csv`, `hti_land_use_change_areas.csv`, `samples_landuse_ttm.csv`, `samples_gfc_ttm.csv`, `centroids_mills_cost.csv`, `grid_10km_adm_prov_kab_kec.csv`, `ws_merge_clean_2015_2022.csv`, `id_annual_expansion_stats_ttm.csv`, etc.). Reviewers need a clear README distinguishing **(a) raw inputs they need**, **(b) intermediate artifacts shipped pre-built**, and **(c) outputs the scripts will create**.
...
- **Folder hygiene**: `01_data/01_in/` has provider-named subfolders (`wwi/`, `klhk/`, `big/`, `gaveau/`, `obidzinski_dermawan/`, `tables/`) — fine, but add a `01_data/01_in/README.md` documenting provenance and licensing for each provider.

Net effect: the bundle should drop from many xlsx + multi-file shapefiles + 1 km nationwide CSVs to a tidy mix of `.csv(.gz)` / `.parquet` / `.gpkg` files, likely a 5–20× reduction in total footprint and a much shorter file count.
